...
\end{codeexample}
  ...
  |(0,0) -- (1,1) circle[radius=2pt] -- (3,2) cirlce[radius=2pt]|.
\end{key}
...
\begin{pathoperation}{circle}{\opt{|[|\meta{options}|]|}}
  This command adds a circle to the currect path where the center of
...
consult this specificaiton for details. 
...
\begin{pathoperation}{svg}{\opt{\oarg{options}}|"|\meta{path data}|"|}
  ...
  \textsc{svg}-specificaiton, it \emph{is} permissble that the path
...
\end{codeexample}
  ...
  the angle is a multiple of $90^\circ$ (as is, fortuntately, most
  often the case).
\end{pathoperation}
